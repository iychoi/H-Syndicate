...
import hsyndicate.hadoop.connector.utils.HSyndicateCompressionCodecUtil;
...
public class HSyndicateLineRecordReader extends RecordReader<LongWritable, Text> {

    private static final Log LOG = LogFactory.getLog(HSyndicateLineRecordReader.class);
    ...
    public HSyndicateLineRecordReader() {
    }

    public HSyndicateLineRecordReader(byte[] recordDelimiter) {
        this.recordDelimiterBytes = recordDelimiter;
    }

    @Override
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException, InterruptedException {
        if (!(genericSplit instanceof HSyndicateInputSplit)) {
            throw new IllegalArgumentException("Creation of a new RecordReader requires a HSyndicateInputSplit instance.");
        }

        HSyndicateInputSplit split = (HSyndicateInputSplit) genericSplit;
