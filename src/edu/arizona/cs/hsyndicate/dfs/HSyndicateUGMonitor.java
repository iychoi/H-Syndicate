...
public class HSyndicateUGMonitor {
    ...
    private static Hashtable<String, AHSyndicateFileSystemBase> syndicateFSs = new Hashtable<String, AHSyndicateFileSystemBase>();
    
    public HSyndicateUGMonitor(Configuration conf) throws IOException {
        String[] gateway_hostnames = HSyndicateConfigUtils.listHSyndicateUGHostsWithPort(conf);
        ...
        for(int i=0;i<gateway_hostnames.length;i++) {
            ...
            if(!syndicateFSs.containsKey(gateway_hostname)) {
                ...
                AHSyndicateFileSystemBase fs = createHSyndicateFS(conf, gateway_hostname);
                syndicateFSs.put(gateway_hostname, fs);
            }
        }
    }
    
    private static AHSyndicateFileSystemBase createHSyndicateFS(Configuration conf, String address) throws IOException {
    ...
    public synchronized List<HSyndicateUGMonitorResults<byte[]>> getLocalCachedBlockInfo(SyndicateFSPath path) throws IOException {
        ...
        for(String gateway_hostname : usergateway_hostnames) {
            AHSyndicateFileSystemBase fs = syndicateFSs.get(gateway_hostname);
            if(fs != null) {
