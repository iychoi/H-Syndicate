...
public class HSyndicateMultipleOutputs<KEYOUT, VALUEOUT> {
     ...
     */
    private static final String COUNTERS_GROUP = HSyndicateMultipleOutputs.class.getName();
    /**
    ...
    private static void checkBaseOutputPath(String outputPath) {
        if (outputPath.equals(HSyndicateFileOutputFormat.PART)) {
     ...
     */
    public HSyndicateMultipleOutputs(
            TaskInputOutputContext<?, ?, KEYOUT, VALUEOUT> context) {
        this.context = context;
        namedOutputs = Collections.unmodifiableSet(new HashSet<String>(HSyndicateMultipleOutputs.getNamedOutputsList(context)));
    ...
    private synchronized RecordWriter getRecordWriter(TaskAttemptContext taskContext, String baseFileName) throws IOException, InterruptedException {
        ...
        if (writer == null) {
            ...
            HSyndicateFileOutputFormat.setOutputName(taskContext, baseFileName);
            try {
     ...
     * overridden subclasses must invoke
     * <code>super.close()</code> at the end of their
     * <code>close()</code>
     *
     */
    @SuppressWarnings("unchecked")
