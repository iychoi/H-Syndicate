 ...
 * <code>data</code> file, containing all keys and values in the map, and a
 * smaller
 * <code>index</code> file, containing a fraction of the keys. The fraction is
 * determined by {@link Writer#getIndexInterval()}.
 *
 ...
 * SequenceFile.Sorter}.
 */
public class HSynthMapFile {

    private static final Log LOG = LogFactory.getLog(HSynthMapFile.class);
    /**
    ...
    public static class Writer implements java.io.Closeable {
        ...
        private HSynthSequenceFile.Writer data;
        private HSynthSequenceFile.Writer index;
        ...
        public Writer(Configuration conf, HSynthFileSystem fs, String dirName,
            ...
            this(conf, fs, dirName,
                    ...
                    HSynthSequenceFile.getCompressionType(conf));
        }
        ...
        public Writer(Configuration conf, HSynthFileSystem fs, String dirName,
                WritableComparator comparator, Class valClass,
                HSynthSequenceFile.CompressionType compress)
                throws IOException {
        ...
        public Writer(Configuration conf, HSynthFileSystem fs, String dirName,
                WritableComparator comparator, Class valClass,
                HSynthSequenceFile.CompressionType compress, CompressionCodec codec,
                Progressable progress)
                throws IOException {
            ...
            HSynthFSPath dir = new HSynthFSPath(dirName);
            fs.mkdirs(dir);
            HSynthFSPath dataFile = new HSynthFSPath(dir, DATA_FILE_NAME);
            HSynthFSPath indexFile = new HSynthFSPath(dir, INDEX_FILE_NAME);

            Class keyClass = comparator.getKeyClass();
            this.data =
                    HSynthSequenceFile.createWriter(fs, conf, dataFile, keyClass, valClass, compress, codec, progress);
            this.index =
                    HSynthSequenceFile.createWriter(fs, conf, indexFile, keyClass, LongWritable.class,
    ...
    public static class Reader implements java.io.Closeable {
        ...
        // the data, on disk
        private HSynthSequenceFile.Reader data;
        private HSynthSequenceFile.Reader index;
         ...
         */
        public Reader(HSynthFileSystem fs, String dirName, Configuration conf) throws IOException {
         ...
         */
        public Reader(HSynthFileSystem fs, String dirName, WritableComparator comparator, Configuration conf)
                throws IOException {
         ...
         */
        protected Reader(HSynthFileSystem fs, String dirName,
        ...
        protected synchronized void open(HSynthFileSystem fs, String dirName,
            ...
            // open the index
            this.index = new HSynthSequenceFile.Reader(fs, indexFile, conf);
        }
         ...
         */
        protected HSynthSequenceFile.Reader createDataFileReader(HSynthFileSystem fs,
                HSynthFSPath dataFile, Configuration conf) throws IOException {
            return new HSynthSequenceFile.Reader(fs, dataFile, conf);
        }
         ...
         *
         * @param before - IF true, and <code>key</code> does not exist,
         * position file at entry that falls just before <code>key</code>.
         ...
         * Returns
         * <code>key</code> or if it does not exist, at the first entry after
         * the named key.
         *         * 
-
         *
         ...
         * the first entry that falls just before the <code>key</code>.
     ...
     */
    public static void rename(HSynthFileSystem fs, String oldName, String newName)
            throws IOException {
        HSynthFSPath oldDir = new HSynthFSPath(oldName);
        HSynthFSPath newDir = new HSynthFSPath(newName);
    ...
    public static void delete(HSynthFileSystem fs, String name) throws IOException {
        ...
        HSynthFSPath data = new HSynthFSPath(dir, DATA_FILE_NAME);
        HSynthFSPath index = new HSynthFSPath(dir, INDEX_FILE_NAME);
     ...
     * @param dryrun do not perform any changes, just report what needs to be
     * done
     * @return number of valid entries in this MapFile, or -1 if no fixing was
     * needed
     * @throws Exception
     */
    public static long fix(HSynthFileSystem fs, HSynthFSPath dir,
        ...
        HSynthSequenceFile.Reader dataReader = new HSynthSequenceFile.Reader(fs, data, conf);
        ...
        HSynthSequenceFile.Writer indexWriter = null;
        if (!dryrun) {
            indexWriter = HSynthSequenceFile.createWriter(fs, conf, index, keyClass, LongWritable.class);
        }
        try {
            long pos = 0L;
            LongWritable position = new LongWritable();
