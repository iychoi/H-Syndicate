...
public abstract class HSyndicateFileOutputFormat<K, V> extends OutputFormat<K, V> {

    private static final Log LOG = LogFactory.getLog(HSyndicateFileOutputFormat.class);
    ...
    private HSyndicateOutputCommitter committer = null;
    ...
    public SyndicateFSPath getDefaultWorkFile(TaskAttemptContext context, String extension) throws IOException {
        HSyndicateOutputCommitter committer = (HSyndicateOutputCommitter) getOutputCommitter(context);
    ...
    public OutputCommitter getOutputCommitter(TaskAttemptContext context) throws IOException {
        if (this.committer == null) {
            ...
            AHSyndicateFileSystemBase fs = null;
            try {
                SyndicateFSConfiguration sconf = hsyndicate.hadoop.utils.HSyndicateConfigUtils.createSyndicateConf(context.getConfiguration(), "localhost");
                fs = SyndicateFileSystemFactory.getInstance(sconf);
            } catch (InstantiationException ex) {
                throw new IOException(ex);
            }
            
            this.committer = new HSyndicateOutputCommitter(fs, output, context);
        }
