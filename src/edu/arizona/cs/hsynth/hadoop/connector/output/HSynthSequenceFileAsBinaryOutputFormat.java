...
import edu.arizona.cs.hsynth.hadoop.connector.io.HSynthSequenceFile.ValueBytes;
 ...
 */
public class HSynthSequenceFileAsBinaryOutputFormat 
    extends HSynthSequenceFileOutputFormat <BytesWritable,BytesWritable> {
  ...
  public RecordWriter<BytesWritable, BytesWritable> getRecordWriter(
    ...
    final HSynthSequenceFile.Writer out = getSequenceWriter(context,
  ...
  protected HSynthSequenceFile.Writer getSequenceWriter(TaskAttemptContext context,
    ...
    try {
        HSynthFSConfiguration fsconf = HSynthConfigUtil.getHSynthFSConfigurationInstance(conf);
        fs = fsconf.getContext().getFileSystem();
    } catch (InstantiationException ex) {
        throw new IOException(ex);
    }
    return HSynthSequenceFile.createWriter(fs, conf, file,
             keyClass,
             valueClass,
             compressionType,
             codec,
