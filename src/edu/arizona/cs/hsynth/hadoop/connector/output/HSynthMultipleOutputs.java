...
public class HSynthMultipleOutputs<KEYOUT, VALUEOUT> {
     ...
     */
    private static final String COUNTERS_GROUP = HSynthMultipleOutputs.class.getName();
    /**
    ...
    private static void checkBaseOutputPath(String outputPath) {
        if (outputPath.equals(HSynthFileOutputFormat.PART)) {
     ...
     */
    public HSynthMultipleOutputs(
            TaskInputOutputContext<?, ?, KEYOUT, VALUEOUT> context) {
        this.context = context;
        namedOutputs = Collections.unmodifiableSet(new HashSet<String>(HSynthMultipleOutputs.getNamedOutputsList(context)));
    ...
    private synchronized RecordWriter getRecordWriter(TaskAttemptContext taskContext, String baseFileName) throws IOException, InterruptedException {
        ...
        if (writer == null) {
            ...
            HSynthFileOutputFormat.setOutputName(taskContext, baseFileName);
            try {
