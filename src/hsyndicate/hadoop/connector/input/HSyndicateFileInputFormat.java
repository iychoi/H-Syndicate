...
public abstract class HSyndicateFileInputFormat<K extends Object, V extends Object> extends InputFormat<K, V> {

    private static final Log LOG = LogFactory.getLog(HSyndicateFileInputFormat.class);
    
    private static final double SPLIT_SLOP = 1.1;   // 10% slop
    
    private static final String CONF_PATH_FILTER = "hsyndicate.conf.input.pathFilter.class";
    private static final String CONF_MIN_SPLIT_SIZE = "hsyndicate.conf.input.min.split.size";
    private static final String CONF_MAX_SPLIT_SIZE = "hsyndicate.conf.input.max.split.size";
        ...
        for(SyndicateFSPath path : files) {
            ...
            if ((length != 0) && isSplitable(context, path)) {
                ...
                while (((double) bytesLeft) / splitSize > SPLIT_SLOP) {
                    HSyndicateInputSplit inputSplit = new HSyndicateInputSplit(fs, path, length - bytesLeft, splitSize);
                ...
                if (bytesLeft != 0) {
                    HSyndicateInputSplit inputSplit = new HSyndicateInputSplit(fs, path, length - bytesLeft, bytesLeft);
                    splits.add(inputSplit);
                }
            } else if (length != 0) {
                HSyndicateInputSplit inputSplit = new HSyndicateInputSplit(fs, path, 0, length);
                splits.add(inputSplit);
            } else {
                HSyndicateInputSplit inputSplit = new HSyndicateInputSplit(fs, path, 0, length);
                splits.add(inputSplit);
    ...
    public static void setInputPaths(Job job, String commaSeparatedPaths) throws IOException {
        setInputPaths(job, PathUtils.stringToPath(getPathStrings(commaSeparatedPaths)));
    }
